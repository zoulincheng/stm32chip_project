...
{ 
	return (unsigned char)((CharToValue(h)<<4) + CharToValue(l)); 
}


unsigned char ascHex2hex(const char *szHexNum, u_char *pHex, int nLenHex)
{
	int nLenData = 0;
	unsigned char data[16] = {0x00};
	unsigned char dataT[16] = {0x00};
	int i = 0; 

	if (nLenHex== 0)
	{
		nLenHex = strlen(szHexNum);
	}

	//XPRINTF((0, "len is %d\r\n", nLenHex));
	for (i = 0; i < nLenHex && IsHexChar(szHexNum[i]); i++)
	{
		data[i] = CharToValue(szHexNum[i])&0x0f;
		//PRINTF("%02x ", data[i]);
	}
	//MEM_DUMP(0, "dadd", data, nLenHex);
	if (nLenHex%2 == 0)
	{
		nLenData = nLenHex >> 1;
		memcpy(dataT, data, nLenHex);
	}
	else 
	{
		nLenData = (nLenHex+1) >> 1;
		memcpy(&dataT[1],data, nLenHex);
		
	}
	//MEM_DUMP(0, "mDDR", dataT, nLenData<<1);
	i = 0;
	//XPRINTF((0," nl %02x\r\n", nLenData));
	for (i = 0; i < nLenData; i++)
	{
		pHex[nLenData-i-1] = dataT[i*2+1]|((dataT[i*2])<<4); 
	}
	//MEM_DUMP(0, "ADDR", pHex, nLenData);
	return nLenData;
}
...
u_long AscBcd2BcdHex(const char *szBcdNum, u_char *pBcdHex, int nLenBcd)
{
	...
	for (i = 0; i < nLenBcd &&isxdigit(szBcdNum[i]); i++)
	{
		...
		*/
		ubaBcdHex[i] = Asc2_OneByteBCD((u_char const *)&ch);
...
void SetNodeID(u_char*pBuf,char cPad,const char *szMeterID,int nIDLen)
{
	...
	//putstrxy(1,1,ID);
	//putstr("\r\n");
	pBuf[0] = Asc2BCD((u_char const *)&ID[10]);
	pBuf[1] = Asc2BCD((u_char const *)&ID[8]);
	pBuf[2] = Asc2BCD((u_char const *)&ID[6]);
	pBuf[3] = Asc2BCD((u_char const *)&ID[4]);
	pBuf[4] = Asc2BCD((u_char const *)&ID[2]);
	pBuf[5] = Asc2BCD((u_char const *)&ID[0]);
}
#else
//cPad: '0','9','A'
void SetNodeID(u_char*pBuf,char cPad,const char *szMeterID,int nIDLen)
...
{
	static const u_short wTable[256]=
	{     /* CRC余式表 */
