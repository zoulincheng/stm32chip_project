...
typedef void(*uart_interrpt_process)(unsigned char *pbuf, unsigned char ubData);


#define RECEIVE_QUEUE_SIZE (128)


typedef  struct _RINGBUFQUEUE
{
	//uint16_t	tri, twi, tct;
	uint8_t		rbuf[RECEIVE_QUEUE_SIZE];
...
//CONTIKIDEVICE* UART_DeviceCreate(int niPort, const USART_InitTypeDef*piInit);
int Uart_Std_Init(void);
//FILE* Uart_GetStdStream(void);
//CONTIKIDEVICE * Uart_GetStdDev(void);
