...
/*---------------------------------------------------------------------------*/
PROCESS(hwgg_rf_process,"hwgg_rf");
...
/*----------------------------------------------------------------------*/
#define END_NODE_RXBUFS	 	16
#define END_NODE_BUF_LEN	32
#define END_NODE_RSSI_INDEX	(END_NODE_BUF_LEN-1)

static uint8_t end_node_rxbufs[END_NODE_RXBUFS][END_NODE_BUF_LEN];

#if END_NODE_RXBUFS > 1
static volatile int8_t end_node_first = -1, end_node_last = 0;
#else   
static const int8_t end_node_first = 0, end_node_last = 0;
#endif  

#if END_NODE_RXBUFS > 1
#define CLEAN_END_NODE_RXBUFS() do{end_node_first = -1; end_node_last = 0;}while(0)
#define END_NODE_RXBUFS_EMPTY() (end_node_first == -1)
static int END_NODE_RXBUFS_FULL( )
{
	int8_t first_tmp = end_node_first;
	return first_tmp == end_node_last;
}
#else 
#define CLEAN_END_NODE_RXBUFS( ) (end_node_rxbufs[0][0] = 0)
#define END_NODE_RXBUFS_EMPTY( ) (end_node_rxbufs[0][0] == 0)
#define END_NODE_RXBUFS_FULL( )  (end_node_rxbufs[0][0] != 0)
#endif 
...
/*---------------------------------------------------------------------------*/
static int add_to_end_node_rxbuf(uint8_t *src)
{
	if(END_NODE_RXBUFS_FULL()) 
	{
		return 0;
	}

	memcpy(end_node_rxbufs[end_node_last], src, src[0]);//src[0] is data length include total packet length 
	end_node_rxbufs[end_node_last][END_NODE_RSSI_INDEX] = src[SI446X_CUR_RSSI];
	#if END_NODE_RXBUFS > 1
	end_node_last = (end_node_last + 1) % END_NODE_RXBUFS;
	if(end_node_first == -1) 
	{
		end_node_first = 0;
	}
...
/*---------------------------------------------------------------------------*/
static int read_from_end_node_rxbuf(void *dest, unsigned short len)
{
	//PRINTF("first 0   %d\r\n", first);
	u_char  packet_rssi;
	int8_t rssi;
	u_char *pdata = (u_char *)dest;
	if(END_NODE_RXBUFS_EMPTY()) 
	{          
		return 0;
	}

	if(end_node_rxbufs[end_node_first][0] > len) 
	{   /* Too large packet for dest. */
		len = 0;
	} 
	else 
	{
		len = end_node_rxbufs[end_node_first][0]-1;  //frame length
		packet_rssi = end_node_rxbufs[end_node_first][END_NODE_RSSI_INDEX];
		rssi = -((int8_t)((0xff-packet_rssi)>>1));
		XPRINTF((10, "prssi %d last_rssi %d  rssi %d\r\n", packet_rssi, last_rssi, rssi));
		memcpy(dest, (uint8_t*)&end_node_rxbufs[end_node_first][0] + 1, len);//copy frame to buf
		pdata[END_NODE_RSSI_INDEX] = packet_rssi;		
	}

	#if END_NODE_RXBUFS > 1
	{
		int first_tmp;
		end_node_first = (end_node_first + 1) % END_NODE_RXBUFS;
		first_tmp = end_node_first;
		if(first_tmp == end_node_last) 
		{
			CLEAN_END_NODE_RXBUFS();
		}
	}
	#else
	CLEAN_END_NODE_RXBUFS();
...
void end_node_frame_save(uint8_t *src)
{
	u_char ubLen = src[0];
	if (src[1] == HWGG_HEAD && src[ubLen-1] == HWGG_END)
	{
		if (isMyPacket((const u_char *)&src[1]))
		{
			add_to_end_node_rxbuf(src);
			process_poll(&hwgg_rf_process);
		}
	}
}
...
PROCESS_THREAD(si446x_radio_process, ev, data)
{
	...
	process_start(&hwgg_rf_process,NULL);
...
/******************************************************************/
PROCESS_THREAD(hwgg_rf_process, ev, data)
{
	int len;
	static u_char ubHwggBuf[32];

	PROCESS_BEGIN();
	XPRINTF((10,"hwgg_rf_process: started\r\n"));
	
	while(1) 
	{
		PROCESS_YIELD_UNTIL(ev == PROCESS_EVENT_POLL);
		len = read_from_end_node_rxbuf(ubHwggBuf, END_NODE_BUF_LEN);
		if(len > 0) 
		{
			MEM_DUMP(6, "<-en", ubHwggBuf, len);
			process_post_synch(&app_hwgg_process, app_hwgg_event, ubHwggBuf);
		}
		if(!END_NODE_RXBUFS_EMPTY()) 
		{
			process_poll(&hwgg_rf_process);
