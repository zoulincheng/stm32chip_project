...
static void SI446X_WAIT_CTS( void )
{
    ...
    do
    {
        ...
        RF_SPINSEL(1);
        cnt++;
        if (cnt > SI446X_WAIT_CTS_CNT)
        {
        	//break;
        }
    }while((cts != 0xFF));
    //}while((cts != 0xFF)&&(cnt < SI446X_WAIT_CTS_CNT));
    //XPRINTF((10, "CNT %d \n", cnt));
}
...
=================================================================================
*/
static void SI446X_PART_INFO( INT8U *buffer )
{
    INT8U cmd = PART_INFO;
...
void si446xRadioInit(void)
{
	...
	rfchnum = pcDevParamInfo->st1NodeConfig.ubRFChannel;
...
int si446xRadioSendFixLenDataHwgg(const INT8U *pubdata)
{
	INT8U ubDataNLen = 0;
	const INT8U *pbuf = pubdata;
	int nResult = 0;
