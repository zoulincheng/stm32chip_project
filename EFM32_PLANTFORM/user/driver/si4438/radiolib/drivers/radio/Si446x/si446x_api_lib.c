...
void si446x_reset(void)
{
    ...
    //for (loopCount = 1000000; loopCount != 0; loopCount--);
   	clock_wait(1);
    radio_hal_DeassertShutdown();
    clock_wait(16);
    //for (loopCount = 72000000; loopCount != 0; loopCount--);
    radio_comm_ClearCTS();
}
...
U8 si446x_configuration_init(const U8* pSetPropCmd)
{
  ...
  while (*pSetPropCmd != 0x00)
  {
	...
	MEM_DUMP(0, "CMD", Pro2Cmd, numOfBytes);
...
void si446x_gpio_pin_cfg(U8 GPIO0, U8 GPIO1, U8 GPIO2, U8 GPIO3, U8 NIRQ, U8 SDO, U8 GEN_CONFIG)
{
    ...
    Si446xCmd.GPIO_PIN_CFG._GPIO[0]        = Pro2Cmd[0];
    Si446xCmd.GPIO_PIN_CFG._GPIO[1]        = Pro2Cmd[1];
    Si446xCmd.GPIO_PIN_CFG._GPIO[2]        = Pro2Cmd[2];
    Si446xCmd.GPIO_PIN_CFG._GPIO[3]        = Pro2Cmd[3];
...
/*
* \breif set PREAMBLE_TX_LENGTH
* note user add                    
* param ubLength set preamble tx length
*/

void si446x_set_preamble_tx_length(U8 ubLength)
{
	
	Pro2Cmd[0] = SI446X_CMD_ID_SET_PROPERTY;	//set cmd
    Pro2Cmd[1] = 0x10;							//group id
    Pro2Cmd[2] = 0x01;							//config data length
    Pro2Cmd[3] = 0x00;							//index
	Pro2Cmd[4] = ubLength;						//config preamble_tx_length
	radio_comm_SendCmd(0x05, Pro2Cmd);
}


/*
* \breif set proerty, onece set only one proerty dagta                  
* param  ubGroup   group id
* param  ubIndex   position of the group
* param  ubProerty proerty data need to be set
* note user add  
*/

void si446x_set_one_proerty(U8 ubGroup,U8 ubIndex, U8 ubProerty)
{
	
	Pro2Cmd[0] = SI446X_CMD_ID_SET_PROPERTY;		//set cmd
    Pro2Cmd[1] = ubGroup;							//group id
    Pro2Cmd[2] = 0x01;								//config data length
    Pro2Cmd[3] = ubIndex;							//index
	Pro2Cmd[4] = ubProerty;							//config preamble_tx_length
	radio_comm_SendCmd(0x05, Pro2Cmd);
}
